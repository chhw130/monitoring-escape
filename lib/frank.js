...
import { filterSkipDays, buildThemeResult } from './slotUtils'
...
async function _fetchBookableDates() {
  ...
  await Promise.all([...months].map(async (monthDate) => {
    try {
      ...
      let m
      while ((m = regex.exec(html)) !== null) { bookableDates.add(m[1]) }
    } catch { /* 달력 조회 실패 시 해당 월 건너뜀 */ }
  }))
  return bookableDates
}

// 동시에 여러 테마가 호출해도 달력 요청을 한 번만 보냄
let _bookableDatesPromise = null
function getBookableDates() {
  if (!_bookableDatesPromise) {
    _bookableDatesPromise = _fetchBookableDates().finally(() => { _bookableDatesPromise = null })
  }
  return _bookableDatesPromise
}
...
export async function fetchFrankThemeSlots(themeId, skipDows = new Set()) {
  ...
  const dates = filterSkipDays([...bookableDates], skipDows)
